...
class SpamhamDetectionModel:
    ...
    def predict(self, X: DataFrame) -> DataFrame:
        ...
        try:
            ...
            # The preprocessing_object here is expected to be a CountVectorizer
            # However, during training, get_stemmed_data is called manually before vectorizer.
            # We must replicate that logic here, OR wrap it in a Pipeline properly.
            # Based on the error "NoneType object has no attribute 'lower'", it seems
            # the input X might be None or the vectorization pipeline is missing a step.
            
            # Replicating the stemming logic from DataTransformation.get_stemmed_data
            # This logic should ideally be inside a custom Transformer class in a Pipeline
            import re
            ...
            # Handle single string input wrapped in wait
            # If X is a list or array
            data_list = X
            if isinstance(X, pd.DataFrame):
                 data_list = X.values.flatten().tolist()
            
            for text in data_list:
                if text is None:
                     corpus.append("")
                     continue
                words = re.sub('[^a-zA-Z]', ' ', str(text))
                words = words.lower()
                words = words.split()
            ...
            transformed_feature = self.preprocessing_object.transform(corpus)
